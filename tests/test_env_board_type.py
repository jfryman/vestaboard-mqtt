...
class TestBoardTypeEnvironmentVariable:
    ...
    def test_board_type_standard_lowercase(self):
        ...
        with patch.dict(
            os.environ, {"VESTABOARD_API_KEY": "test_key", "VESTABOARD_BOARD_TYPE": "standard"}
        ):
            config = AppConfig.from_env()
            client = create_vestaboard_client(config=config.vestaboard)
            assert client.board_rows == 6
    ...
    def test_board_type_note_lowercase(self):
        ...
        with patch.dict(
            os.environ, {"VESTABOARD_API_KEY": "test_key", "VESTABOARD_BOARD_TYPE": "note"}
        ):
            config = AppConfig.from_env()
            client = create_vestaboard_client(config=config.vestaboard)
    ...
    def test_board_type_parameter_overrides_env_var(self):
        ...
        with patch.dict(
            os.environ,
            {"VESTABOARD_API_KEY": "test_key", "VESTABOARD_BOARD_TYPE": "note"},  # Set to note
        ):
    ...
    def test_invalid_board_type_raises_error(self):
        ...
        with patch.dict(
            os.environ, {"VESTABOARD_API_KEY": "test_key", "VESTABOARD_BOARD_TYPE": "invalid_type"}
        ):
    ...
    def test_empty_board_type_defaults_to_standard(self):
        ...
        with patch.dict(
            os.environ, {"VESTABOARD_API_KEY": "test_key", "VESTABOARD_BOARD_TYPE": ""}
        ):
            config = AppConfig.from_env()
            client = create_vestaboard_client(config=config.vestaboard)
            assert client.board_rows == 6
...
class TestBoardTypeWithLocalApi:
    ...
    def test_standard_board_with_cloud_api(self):
        ...
        with patch("src.config.load_dotenv"):
            with patch.dict(
                os.environ,
                {"VESTABOARD_API_KEY": "test_key", "VESTABOARD_BOARD_TYPE": "standard"},
                clear=True,
            ):
                config = AppConfig.from_env()
                client = create_vestaboard_client(config=config.vestaboard)
...
@pytest.mark.parametrize(
    "env_value,expected_rows,expected_cols",
    [
        ("standard", 6, 22),
        ("STANDARD", 6, 22),
        ("note", 3, 15),
...
def test_board_type_env_parsing_parametrized(env_value, expected_rows, expected_cols):
    ...
    with patch.dict(
        os.environ, {"VESTABOARD_API_KEY": "test_key", "VESTABOARD_BOARD_TYPE": env_value}
    ):
        config = AppConfig.from_env()
        client = create_vestaboard_client(config=config.vestaboard)
